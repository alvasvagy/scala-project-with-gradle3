...
import org.homeworks.Homework7.SomeEnum
import org.homeworks.ScalaEnumByEnumWithOverride.Colors
...
object ScalaEnumByEnumWithOverride extends App {
  ...
  val cc = Colors.withName("Red color").id
  println(cc)
...
object Homework7 extends App{
  object SomeEnum extends Enumeration {
    val First, Second, Three, Four = Value
  }
  // не понял почему фор выкидывает sortedSet, ну да ладн
  def betweenGetter(first:Int, second:Int) = {
    for { i <- SomeEnum.values if (i.id > first && i.id < second)} yield i
  }
  betweenGetter(0, 6).foreach(println)

  object WeekDays extends Enumeration {
    type Day = Value
    val Monday                            = Value(1, "Montag")
    val Tuesday                           = Value(2, "Dienstag")
    val Wednesday                         = Value(3, "Mittwoch")
    val Thursday                          = Value(4, "Donnerstag")
    val Friday                            = Value(5, "Freitag")
    val Saturday                          = Value(6, "Samstag")
    val Sunday                            = Value(7, "Sontag")
    def nextDay(c: WeekDays.Day): Value = {
      c match {
        case c if c.id != 7 => WeekDays(c.id + 1)
        case _ => WeekDays(1)
      }
  ...
  println(WeekDays.nextDay(WeekDays.Monday))
  println(WeekDays.nextDay(WeekDays.Sunday))

  def filtList(l: List[Int]): List[Int] = {
    l.distinct.toList
  }

  def meanOptionList(l: List[Option[Int]]): Double = {
    var all:Int = 0
    val filtered = l.filter(i => !i.isEmpty)
    filtered.foreach(i => all = all + i.get) // не знаю как чистой функциональщиной это сделать
    all / filtered.length
  }

  def fiveStringOptionFilter(l: List[Option[String]]): Unit = {
    l.count(i => i.getOrElse("").length > 5)
  }

  def conjunctionString(l1:List[String], l2:List[String]): Seq[String] = {
    l1.filter(i => l2.contains(i))
  }

  def combinationsInt(l1:List[Int], l2:List[Int]): List[List[(Int, Int)]] = {
    for (i <- l1) yield for (j <- l2) yield {(i, j)}
  }
  println(combinationsInt(List[Int](2, 3, 5), List(2, 2, 2)))
  // на выходе, всевозможные неупорядочные пары ( (5,2) = (2,5) )
}
