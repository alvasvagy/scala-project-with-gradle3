...
object Lec8Map extends App {
  ...
  val a22: Map[Int, String] = map1.map { x =>
    val nK = x._1 + 1
    val nV = x._2 ++ "!!!"
    nK -> nV // (nK, nV)
  }
...
object Homework8 extends App{
  def SetUniqueElements(set1:Set[Int], set2:Set[Int]): Set[Int] = {
    val first: Set[Int] = for {i <- set1 if(!set2.contains(i))} yield i
    val second: Set[Int] = for {i <- set2 if(!set1.contains(i))} yield i
    first ++ second
  }
  println(SetUniqueElements(Set(1, 3, 5), Set(2, 5, 7)))


  def doubleOptionMap(key:String, value:Option[Int]): Map[String, Int] = {
    Map(key -> value.get * 2)
  }
  println(doubleOptionMap("hell", Option(666)))

  def mapPlusOne(mapa:Map[String, Int]): Map[String, Int] = {
    mapa.map( x => (x._1, x._2 + 1)  )
  }
